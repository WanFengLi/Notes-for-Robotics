...
先解方程组
$\begin{cases}
l(x-x_0)+m(y-y_0)+n(z-z_0)=0\\ \frac{x-x_0}{l}=\frac{y-y_0}{m}=\frac{z-z_0}{n}\\ \end{cases}$得结果 $x_p,y_p,z_p$
其距离为
...
**方程形式**可表示为: 
$\begin{cases} x=x(t)\\ y=y(t)\\ z=z(t)\\ \end{cases}$ ；或写成两个(曲面)方程的非线性方程组(不含 t)
